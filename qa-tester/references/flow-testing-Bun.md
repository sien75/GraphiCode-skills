...
## Test structure (Bun test runner)
...
describe('FlowName', () => {
  ...
  test('connection #0: on EventA, pipe through algo, call methodB', () => {
...
```ts
test('method executes only when all params received', () => {
...
```ts
test('then routes return value to target', async () => {
...
test('catch routes error to target', async () => {
...
```ts
test('then broadcasts event to EventBus', () => {
...
For Bun environments, use `bun test`:
